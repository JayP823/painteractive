...
async function register(userParam, files){
    ...
    if(!files){
        file.avatar = [];
        file.header = [];
        file.avatar.push({filename: "484bf2658cf3dc5e8108f5887080c515.png"});
        file.header.push({filename: "5ef0c024fa713ca7f961a3abe1f75969.jpg"});
    } else {
        file = files;
    }
...
async function getUserPosts(req){
    ...
    let user2 = user[0];
    let post = await Post.find({$or: [{createdBy: user2._id}, {reposted: user2._id}]}).sort({createdDate: -1}).skip(skipNum).limit(12).populate({path:'createdBy'}).populate({path: 'comments.user'}).populate('image');
...
async function getUserLikedPosts(req){
    ...
    let user = await User.find({username: req.query.username});
    let user2 = user[0]
    return await Post.find({liked: user2._id}).sort({createdDate: -1}).populate({path:'createdBy'}).populate({path: 'comments.user'}).populate('image').skip(skipNum).limit(12);
}
...
async function getMedia(req){
    ...
    let user = await User.find({username: req.query.username});
    let user2 = user[0];
    return await Post.find({$and: [{$or: [{createdBy: user2._id}, {reposted: user2._id}]}, {image: {$exists: true}}]}).sort({createdDate: -1}).populate({path:'createdBy'}).populate({path: 'comments.user'}).populate('image').skip(skipNum).limit(12);
}
